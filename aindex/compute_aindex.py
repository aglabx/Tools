...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', help='Reads type reads, fasta, fastq, se [fastq]', required=False, default="fastq")
    ...
    parser.add_argument('--interactive', help='Interactive [None]', required=False, default=None)
    ...
    parser.add_argument('--onlyindex', help='Compute only index [False]', required=False, default=False)
    parser.add_argument('--unzip', help='Unzip files [False]', required=False, default=False)
    parser.add_argument('--kmers', help='Make kmers file [False]', required=False, default=False)
    parser.add_argument('--path_to_aindex', help='Path to aindex folder inside Tools [~/Dropbox/workspace/PyBioSnippets/Tools/aindex]', required=False, default="~/Dropbox/workspace/PyBioSnippets/Tools/aindex")
    ...
    ## Add format resolution postfix, depending on input format
    if reads_type == 'fastq':
        reads_file = ",".join([f"{file}.fastq" for file in reads_file.split(',')])
    elif reads_type == 'fasta':
        reads_file = ",".join([f"{file}.fasta" for file in reads_file.split(',')])
    elif reads_type == 'se':
        reads_file = f"{reads_file}.fastq"
    
    required_files = [reads_file.split(",")]
    missing_files = [file for file in required_files if file and not os.path.exists(file)]
    ...
    if not jf2_file and not index_prefix:
        if reads_type == "reads":
            ...
            commands = [
                f"jellyfish count -m 23 -t %s -s %sG -C -L %s -o %s.23.jf2 %s" % (threads, memory, lu, prefix, reads_file.replace(",", " ")),
            ]
            runner(commands)
            if interactive:
                input("Continue?")
    ...
    if not only_index:
        if reads_type == "fasta":
            if not build_header:
                commands = [
                    f"python {path_to_aindex}/fasta_to_reads.py -i {reads_file} -o {prefix}.reads" % (reads_file, prefix),   
                ]                
            else:
                commands = [
